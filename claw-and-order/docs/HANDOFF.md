...
**Status:** Feature-complete, hardened, 38/38 tests passing
...
**Our team built:** The entire backend communication pipeline — FastAPI server, API endpoints, authentication, phase enforcement, milestone detection, scoring engine, SSE streaming, Discord integration, reporter deployment package, operational scripts, and the full test suite.
...
|   |-- technique_map.json      # Command-to-technique mapping rules
|   +-- technique_normalizer.py # Standalone normalizer with tests
|
...
4. **Never modify `reporter.py` core logic** — only the `is_unknown()` addition was approved
